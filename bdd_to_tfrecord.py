...
def _create_tf_record_from_bdd_annotations(annotations_file, image_dir, output_path, num_shards):
    ...
    with contextlib2.ExitStack() as tf_record_close_stack, \
        ...
        for idx, frame in enumerate(groundtruth_data):
            if idx % 100 == 0:
                tf.logging.info('On image {} of {}'.format(idx+1, len(image_list)))
...
def main(_):
  ...
  train_output_path = os.path.join(FLAGS.output_dir, 'bdd100k_train.record')
  val_output_path = os.path.join(FLAGS.output_dir, 'bdd100k_val.record')
